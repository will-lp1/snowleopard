  ...
  return (
    <div className="bg-background border rounded-xl w-full max-w-md flex items-center p-3 gap-3 text-sm">
      ...
      <div className="flex-grow text-foreground">
        {t('Searching web for "{query}"...', { query: args.query })}
      </div>
    ...
    <div className="bg-background border rounded-xl w-full max-w-md p-4 text-sm">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span>{t('Search completed for "{query}"', { query: args.query })}</span>
        </div>
        <Button 
          variant="link" 
          size="sm" 
      ...
      {open && (
        list.length === 0 ? (
          <T><p className="text-muted-foreground mt-2">No sources found.</p></T>
        ) : (
